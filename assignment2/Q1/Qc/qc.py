...
if __name__ == "__main__":
    ...
    bp = np.count_nonzero(model.predict(testdata_pos) == 1)
    bn = np.count_nonzero(model.predict(testdata_neg) == 0)

    p, n = len(testdata_pos), len(testdata_neg)
    rp, rn = np.random.binomial(p,0.5), np.random.binomial(n,0.5)

    print(f"      NaiveBayes              Random             AllPositive")
    print(f"   |  AP  |  AN  |       |  AP  |  AN  |       |  AP  |  AN  |")
    print(f"---|------|------|    ---|------|------|    ---|------|------|")
    print(f"PP | {bp:4} | {n-bn:4} |    PP | {rp:4} | {n-rn:4} |    PP | {p:5}| {n:4} |")
    print(f"PN | {p-bp:4} | {bn:4} |    PN | {p-rp:4} | {rn:4} |    PN | {0:4} | {0:4} |")
